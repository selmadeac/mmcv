...
class CosineAnnealingRestartsWithDecayLrUpdaterHook(LrUpdaterHook):
    ...
    def __init__(self, eta_min=1e-5,period=0.5, decay=0.03,**kwargs):
        self.lr_min = eta_min
        # Decay attributes
        self.decay = decay
        self.lr_max =0
    ...
    def get_lr(self,runner, base_lr):
        ...
        if progress==0 :
            self.lr_max = base_lr
            if not self.by_epoch:
                self.period = (runner.max_iters/100)
                print(self.period)
        ...
        lr = self.damp_cosine_wave(progress,self.period,self.decay,self.lr_min,self.lr_max)

        return lr

    def damp_cosine_wave(self,i, p, decay,min_lr,max_lr):
        x =  (1+math.cos(1/math.pi *i/p )) * math.exp(-decay * i/p) 
        x = min_lr+(max_lr-min_lr)*x
        return x

@HOOKS.register_module()
class ReserveSigmoidLrUpdaterHook(LrUpdaterHook):
    r"""Set the learning rate of each parameter group using a cosine annealing
    schedule, where :math:`\eta_{max}` is set to the initial lr, :math:`T_{cur}`
    is the number of epochs since the last restart and :math:`T_{i}` is the number
    of epochs between two warm restarts in SGDR:
    ...
    def __init__(self, eta_min=1e-5,**kwargs):
        self.by_epoch=True
        # Decay attributes
        self.lr_max =0
        self.lr_min=eta_min
    ...
    def get_lr(self,runner, base_lr):
        ...
        lr = self.reversesigmoid(progress,self.lr_min,self.lr_max)

        return lr

    def reversesigmoid(self,i,min_lr,max_lr):
        x= 1/(1+0.0000001*math.pow(5,i*0.8))
        x = min_lr+(max_lr-min_lr)*x
        return x

@HOOKS.register_module()
class CenterformerStepCycLrUpdaterHook(LrUpdaterHook):
    ...
    def __init__(self, **kwargs):  
        ...
        self.target_ratio = (1, 1e-4)
        self.cyclic_times = 1
        self.step_ratio_up = 0.4
        self.gamma = 1
    ...
    def get_lr(self, runner, base_lr):
        #print(runner.epoch,"runner.epoch")
        #print(runner.max_iters ,"self.max_iter_per_phase  ")
        if runner.epoch<self.split_epoch:
    ...
    def before_run(self, runner: 'runner.BaseRunner'):
        ...
        remaining_epochs = runner.max_epochs-self.split_epoch
        self.max_iter_per_phase = (iters_per_epoch*remaining_epochs) // self.cyclic_times

        print(self.max_iter_per_phase,"self.max_iter_per_phase")
...
@HOOKS.register_module()
class CenterformerCosAnLrUpdaterHook(LrUpdaterHook):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.cos1 = CosineAnnealingLrUpdaterHook(min_lr=0.001*10,by_epoch=True,warmup=None)

        self.cos3 = CosineAnnealingLrUpdaterHook(min_lr=0.001* 1e-4,by_epoch=True,warmup=None)

    def get_lr(self, runner, base_lr):
        if runner.epoch<6:
            return self.cos1.get_lr(runner,base_lr)
        else:
            return self.cos3.get_lr(runner,base_lr)
...
@HOOKS.register_module()
class FixMultiplyLrUpdaterHook(LrUpdaterHook):

    def __init__(self, multiply=1, **kwargs):
        super().__init__(**kwargs)
        self.multiply =multiply

    def get_lr(self, runner, base_lr):
        return base_lr*self.multiply
...
class CosineAnnealingLrUpdaterHook(LrUpdaterHook):
    ...
    def get_lr(self, runner: 'runner.BaseRunner', base_lr: float):
        ...
        return annealing_cos(base_lr, target_lr, progress**self.k_decay / max_progress**self.k_decay)
...
@HOOKS.register_module()
class DampedCosineAnnealingRestartLrUpdaterHook(LrUpdaterHook):
    def __init__(self,steps,t0, eta_min=0,ti=1,  damping_factor=0.1,**kwargs):
    ...
    def get_lr(self, runner: 'runner.BaseRunner', base_lr: float):
        ...
        self.ti = runner.epoch

        print(math.exp(-self.damping_factor * (runner.epoch)))
...
class DampedCosineAnnealingLR(LrUpdaterHook):
    def __init__(self,  eta_min=0, damping_factor=0.1,**kwargs):
    ...
    def get_lr(self, runner: 'runner.BaseRunner', base_lr: float):
        cos_inner = math.pi * ((runner.epoch) % runner.max_epochs) / runner.max_epochs
        cos_outer = math.exp(-self.damping_factor * (runner.epoch))
        cos_val = cos_outer * (math.cos(cos_inner) + self.damping_factor * math.sin(cos_inner))
        return self.eta_min + (base_lr - self.eta_min) * (1 + cos_val) / 2
