...
class CenterformerCosAnMomentumUpdaterHook(MomentumUpdaterHook):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.cos1 = CosineAnnealingMomentumUpdaterHook(min_momentum=0.85 / 0.95,by_epoch=True,warmup=None)

        self.cos3 = CosineAnnealingMomentumUpdaterHook(min_momentum=1,by_epoch=True,warmup=None)

    def get_momentum(self, runner, base_lr):
        if runner.epoch<6:
            return self.cos1.get_momentum(runner,base_lr)
        
        else:
            return self.cos3.get_momentum(runner,base_lr)


@HOOKS.register_module()
class CenterformerStepCycMomentumUpdaterHook(MomentumUpdaterHook):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        #self.cos1 = CosineAnnealingMomentumUpdaterHook(min_momentum=0.85 / 0.95,by_epoch=True,warmup=None)

        #self.cos3 = CyclicMomentumUpdaterHook(target_ratio=(0.85 / 0.95, 1),step_ratio_up=0.4,by_epoch=False,cyclic_times=1,warmup=None)
